...
class WpvHtmlChecker {
    ...
    function CleanHtmlTags($string) {
        $checker = new Wpv_SafeHtmlChecker();
        $checker->check('<all>'.$string.'</all>');
        
        if ($checker->isOK()) {
            return $string;
        } 
        else {
            return WpvHtmlChecker::StripHtmlTags($string);
        }
    }
}
...
// MY - Added <b> <span>
define('E_INLINE_CONTENTS', 'em strong b span dfn code q samp kbd var cite abbr acronym sub sup a #PCDATA');
define('E_BLOCK_CONTENTS', 'dl ul ol blockquote p');
define('E_FLOW_CONTENTS', E_BLOCK_CONTENTS.' '.E_INLINE_CONTENTS);

class Wpv_SafeHtmlChecker {
